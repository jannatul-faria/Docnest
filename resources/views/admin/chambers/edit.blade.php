<x-admin-layout>
    @section('header', 'Edit Chamber')

    <div class="max-w-4xl mx-auto pb-12">
        <div class="mb-8 flex items-center justify-between px-2">
            <div>
                <h3 class="text-2xl font-black text-slate-900 tracking-tight">Edit Chamber</h3>
                <p class="text-sm text-slate-500 font-medium">Update clinic or hospital chamber details and visiting schedules.</p>
            </div>
        ...
        <form action="{{ route('admin.chambers.update', $chamber) }}" method="POST" class="space-y-8">
            @csrf
            @method('PUT')
            ...
            <div class="bg-white rounded-[2.5rem] shadow-sm border border-slate-100 p-8 md:p-12">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    ...
                    <div class="space-y-2">
                        ...
                        <input type="text" name="name" id="name" value="{{ old('name', $chamber->name) }}" class="w-full rounded-2xl border-slate-100 bg-slate-50 focus:bg-white focus:ring-4 focus:ring-blue-50 focus:border-blue-400 transition-all font-bold text-slate-700">
                        ...
                        <div class="space-y-2">
                            ...
                            <select name="division_id" id="division_id" class="w-full rounded-2xl border-slate-100 bg-slate-50 focus:bg-white focus:ring-4 focus:ring-blue-50 transition-all font-bold text-slate-700">
                                ...
                                @foreach($divisions as $division)
                                    <option value="{{ $division->id }}" {{ old('division_id', $chamber->division_id) == $division->id ? 'selected' : '' }}>{{ $division->name }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="space-y-2">
                            <label for="district_id" class="text-xs font-black text-slate-400 uppercase tracking-widest ml-1">District</label>
                            <select name="district_id" id="district_id" class="w-full rounded-2xl border-slate-100 bg-slate-50 focus:bg-white focus:ring-4 focus:ring-blue-50 transition-all font-bold text-slate-700" {{ !$chamber->district_id ? 'disabled' : '' }}>
                                <option value="">Select District</option>
                                @foreach($districts as $district)
                                    <option value="{{ $district->id }}" {{ old('district_id', $chamber->district_id) == $district->id ? 'selected' : '' }}>{{ $district->name }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="space-y-2">
                            <label for="area_id" class="text-xs font-black text-slate-400 uppercase tracking-widest ml-1">Area / Police Station</label>
                            <select name="area_id" id="area_id" class="w-full rounded-2xl border-slate-100 bg-slate-50 focus:bg-white focus:ring-4 focus:ring-blue-50 focus:border-blue-400 transition-all font-bold text-slate-700" {{ !$chamber->district_id ? 'disabled' : '' }}>
                                <option value="">Select Area</option>
                                @foreach($areas as $area)
                                    <option value="{{ $area->id }}" {{ old('area_id', $chamber->area_id) == $area->id ? 'selected' : '' }}>{{ $area->name }}</option>
                                @endforeach
                            </select>
                    ...
                    <div class="md:col-span-2 space-y-2">
                        ...
                        <textarea name="address" id="address" rows="3" class="w-full rounded-2xl border-slate-100 bg-slate-50 focus:bg-white focus:ring-4 focus:ring-blue-50 focus:border-blue-400 transition-all font-bold text-slate-700">{{ old('address', $chamber->address) }}</textarea>
                    ...
                    <div class="space-y-2">
                        ...
                        <input type="text" name="phone" id="phone" value="{{ old('phone', $chamber->phone) }}" class="w-full rounded-2xl border-slate-100 bg-slate-50 focus:bg-white focus:ring-4 focus:ring-blue-50 focus:border-blue-400 transition-all font-bold text-slate-700">
                    ...
                    <div class="space-y-2">
                        ...
                        <select name="status" id="status" class="w-full rounded-2xl border-slate-100 bg-slate-50 focus:bg-white focus:ring-4 focus:ring-blue-50 focus:border-blue-400 transition-all font-bold text-slate-700">
                            <option value="1" {{ old('status', $chamber->status) == '1' ? 'selected' : '' }}>Currently Open</option>
                            <option value="0" {{ old('status', $chamber->status) == '0' ? 'selected' : '' }}>Temporarily Closed</option>
                        </select>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white rounded-[2.5rem] shadow-sm border border-slate-100 p-8 md:p-12">
                ...
                <div id="schedule-container" class="space-y-4">
                    @foreach($chamber->schedules as $index => $schedule)
                        <div class="schedule-row grid grid-cols-1 md:grid-cols-4 gap-4 p-6 bg-slate-50 rounded-3xl border border-slate-100 relative group">
                            <div class="space-y-2">
                                <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Day</label>
                                <select name="schedules[{{ $index }}][day]" class="w-full rounded-xl border-slate-100 bg-white focus:ring-4 focus:ring-blue-50 transition-all font-bold text-slate-700">
                            ...
                            <div class="space-y-2">
                                ...
                                <input type="time" name="schedules[{{ $index }}][start_time]" value="{{ \Carbon\Carbon::parse($schedule->start_time)->format('H:i') }}" class="w-full rounded-xl border-slate-100 bg-white focus:ring-4 focus:ring-blue-50 transition-all font-bold text-slate-700">
                            </div>
                            <div class="space-y-2">
                                <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">End Time</label>
                                <input type="time" name="schedules[{{ $index }}][end_time]" value="{{ \Carbon\Carbon::parse($schedule->end_time)->format('H:i') }}" class="w-full rounded-xl border-slate-100 bg-white focus:ring-4 focus:ring-blue-50 transition-all font-bold text-slate-700">
                            </div>
                            <div class="space-y-2">
                                <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Max Patients</label>
                                <input type="number" name="schedules[{{ $index }}][max_patients]" value="{{ $schedule->max_patients }}" class="w-full rounded-xl border-slate-100 bg-white focus:ring-4 focus:ring-blue-50 transition-all font-bold text-slate-700">
                            </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            let scheduleIndex = {{ count($chamber->schedules) }};
