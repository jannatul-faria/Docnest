<x-admin-layout>
    <x-slot name="header">Districts</x-slot>

    <div class="space-y-4">
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
                <h1 class="text-2xl font-black text-heading tracking-tight">Manage Districts</h1>
                <p class="text-sm text-slate-500 font-medium">Add, edit or remove districts for doctor locations</p>
            </div>
            <a href="{{ route('admin.districts.create') }}" class="inline-flex items-center px-4 py-2 bg-primary text-white rounded-xl text-sm font-bold hover:bg-primary/90 transition-all shadow-md shadow-primary/10">
        ...
        <div class="bg-white p-3 rounded-xl border border-slate-100 shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div class="relative flex-1 max-w-md">
                <form action="{{ route('admin.districts.index') }}" method="GET" class="relative">
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-sm text-left text-slate-500">
                    <thead class="text-[11px] text-slate-400 uppercase tracking-widest bg-slate-50/50 border-b border-slate-100 font-bold">
                        <tr>
                            <th class="px-6 py-4 text-slate-600">District Name</th>
                            <th class="px-4 py-4 text-slate-600">Division</th>
                    ...
                    <tbody class="divide-y divide-slate-100">
                        @forelse($districts as $district)
                            <tr class="hover:bg-slate-50/30 transition-all duration-200">
                                <td class="px-6 py-4">
                                    <div class="font-bold text-slate-900 leading-tight group-hover:text-primary transition-colors">{{ $district->name }}</div>
                                    <div class="text-[10px] font-medium text-slate-400 font-mono tracking-tight uppercase">{{ $district->slug }}</div>
                                </td>
                                <td class="px-4 py-4">
                                ...
                                <td class="px-6 py-4 text-right">
                                    <div class="flex justify-end space-x-1">
                                        <a href="{{ route('admin.districts.edit', $district) }}" class="p-1.5 text-slate-400 hover:text-primary hover:bg-primary/10 rounded-lg transition-all" title="Edit">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>
                                        </a>
                                        <form id="delete-form-{{ $district->id }}" action="{{ route('admin.districts.destroy', $district) }}" method="POST" class="inline">
                                            @csrf
                                            @method('DELETE')
                                            <button type="button" onclick="confirmDelete('delete-form-{{ $district->id }}')" class="p-1.5 text-slate-400 hover:text-rose-600 hover:bg-rose-50 rounded-lg transition-all" title="Delete">
