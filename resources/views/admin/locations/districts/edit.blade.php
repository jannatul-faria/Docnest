    ...
    <div class="max-w-2xl mx-auto">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-slate-100 overflow-hidden">
            <div class="p-6 border-b border-slate-100 bg-slate-50/50">
                <h3 class="text-lg font-medium text-slate-900">Edit District: {{ $district->name }}</h3>
                <p class="text-sm text-slate-500">Update the details for this district.</p>
            </div>

            <form action="{{ route('admin.districts.update', $district) }}" method="POST" class="p-6 space-y-6">
                @csrf
                @method('PUT')
                ...
                <div>
                    ...
                    <select name="division_id" id="division_id" class="w-full rounded-lg border-slate-200 focus:border-blue-500 focus:ring-blue-500 transition-colors @error('division_id') border-rose-500 @enderror">
                        ...
                        @foreach($divisions as $division)
                            <option value="{{ $division->id }}" {{ old('division_id', $district->division_id) == $division->id ? 'selected' : '' }}>{{ $division->name }}</option>
                        @endforeach
                    </select>
                    @error('division_id')
                ...
                <div>
                    ...
                    <input type="text" name="name" id="name" value="{{ old('name', $district->name) }}" placeholder="e.g. Gazipur" class="w-full rounded-lg border-slate-200 focus:border-blue-500 focus:ring-blue-500 transition-colors @error('name') border-rose-500 @enderror">
                    @error('name')
                        <p class="mt-1 text-sm text-rose-500">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <select name="status" id="status" class="w-full rounded-lg border-slate-200 focus:border-blue-500 focus:ring-blue-500 transition-colors">
                        ...
                        <option value="0" {{ old('status', $district->status) == '0' ? 'selected' : '' }}>Inactive</option>
                    </select>
                    @error('status')
                        <p class="mt-1 text-sm text-rose-500">{{ $message }}</p>
                    @enderror
