<x-admin-layout>
    ...
    <div class="space-y-6">
        ...
        <div class="flex justify-between items-center">
            <div>
                <h3 class="text-lg font-medium text-slate-900">Manage Areas</h3>
                <p class="text-sm text-slate-500">Add, edit or remove areas for doctor locations.</p>
            </div>
            <a href="{{ route('admin.areas.create') }}" class="inline-flex items-center px-4 py-2 bg-blue-600 border border-transparent rounded-lg font-semibold text-xs text-white uppercase tracking-widest hover:bg-blue-700 focus:bg-blue-700 active:bg-blue-900 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 transition ease-in-out duration-150 shadow-sm">
                ...
                <table class="w-full text-sm text-left text-slate-500">
                    <thead class="text-xs text-slate-700 uppercase bg-slate-50 border-b border-slate-100">
                        <tr>
                            ...
                            <th scope="col" class="px-6 py-4 font-semibold">District & Division</th>
                        ...
                        @forelse($areas as $area)
                            <tr class="hover:bg-slate-50/50 transition-colors">
                                <td class="px-6 py-4">
                                    ...
                                    <div class="text-xs text-slate-400 font-mono">{{ $area->slug }}</div>
                                </td>
                                <td class="px-6 py-4">
                                ...
                                <td class="px-6 py-4 text-right">
                                    <div class="flex justify-end space-x-2">
                                        <a href="{{ route('admin.areas.edit', $area) }}" class="p-2 text-blue-600 hover:bg-blue-50 rounded-lg transition-colors" title="Edit">
                                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>
                                        </a>
                                        <form id="delete-form-{{ $area->id }}" action="{{ route('admin.areas.destroy', $area) }}" method="POST">
                                            @csrf
                                            @method('DELETE')
                                            <button type="button" onclick="confirmDelete('delete-form-{{ $area->id }}', 'Are you sure you want to delete this area? This action cannot be undone.')" class="p-2 text-rose-600 hover:bg-rose-50 rounded-lg transition-colors" title="Delete">
                            ...
                            <tr>
                                <td colspan="4" class="px-6 py-10 text-center">
                                    <div class="flex flex-col items-center">
                                        ...
                                        <p class="text-slate-500">No areas found. Create your first one!</p>
                                    </div>
                                </td>
                            </tr>
                        @endforelse
