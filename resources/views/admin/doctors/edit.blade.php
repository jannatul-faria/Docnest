<x-admin-layout>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-slate-100 overflow-hidden">
            ...
            <div class="p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="text" name="name" id="name" value="{{ old('name', $doctor->name) }}" class="w-full rounded-lg border-slate-200 focus:border-blue-500 focus:ring-blue-500 @error('name') border-red-600 @enderror">
                    @error('name') <p class="mt-1 text-sm text-red-600">{{ $message }}</p> @enderror
                </div>
                <div>
                    <label for="email" class="block text-sm font-medium text-slate-700 mb-1">Email Address</label>
                    <input type="email" name="email" id="email" value="{{ old('email', $doctor->email) }}" class="w-full rounded-lg border-slate-200 focus:border-blue-500 focus:ring-blue-500 @error('email') border-red-600 @enderror">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            <div class="bg-white rounded-xl shadow-sm border border-slate-100 overflow-hidden">
                ...
                <div id="education-container" class="p-4 space-y-4">
                    @forelse($doctor->educations as $index => $edu)
                        <div class="education-row p-3 bg-slate-50 rounded-lg border border-slate-100 relative">
                            ...
                            <div class="space-y-3">
                                <input type="text" name="educations[{{ $index }}][degree]" value="{{ $edu->degree }}" placeholder="Degree (e.g. MBBS)" class="w-full text-sm rounded-lg border-slate-200">
                                <input type="text" name="educations[{{ $index }}][institution]" value="{{ $edu->institution }}" placeholder="Institution" class="w-full text-sm rounded-lg border-slate-200">
                                <input type="text" name="educations[{{ $index }}][passing_year]" value="{{ $edu->passing_year }}" placeholder="Year" class="w-full text-sm rounded-lg border-slate-200">
                            </div>
                        </div>
                    @empty
                        <div class="education-row p-3 bg-slate-50 rounded-lg border border-slate-100 relative">
            ...
            <div class="bg-white rounded-xl shadow-sm border border-slate-100 overflow-hidden">
                ...
                <div id="experience-container" class="p-4 space-y-4">
                    @forelse($doctor->experiences as $index => $exp)
                        <div class="experience-row p-3 bg-slate-50 rounded-lg border border-slate-100 relative">
                            <button type="button" onclick="this.parentElement.remove()" class="absolute -top-2 -right-2 bg-rose-500 text-white rounded-full p-1 shadow-sm">
                                <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path></svg>
                            </button>
                            <div class="space-y-3">
                                <input type="text" name="experiences[{{ $index }}][designation]" value="{{ $exp->designation }}" placeholder="Designation" class="w-full text-sm rounded-lg border-slate-200">
                                <input type="text" name="experiences[{{ $index }}][institution]" value="{{ $exp->institution }}" placeholder="Institution" class="w-full text-sm rounded-lg border-slate-200">
                                <div class="grid grid-cols-2 gap-2">
                                    <input type="date" name="experiences[{{ $index }}][start_date]" value="{{ $exp->start_date ? \Carbon\Carbon::parse($exp->start_date)->format('Y-m-d') : '' }}" class="w-full text-sm rounded-lg border-slate-200">
                                    <input type="date" name="experiences[{{ $index }}][end_date]" value="{{ $exp->end_date ? \Carbon\Carbon::parse($exp->end_date)->format('Y-m-d') : '' }}" class="w-full text-sm rounded-lg border-slate-200">
                                </div>
                            </div>
                        </div>
                    @empty
    ...
    @push('scripts')
    <script>
        let eduCount = {{ count($doctor->educations) > 0 ? count($doctor->educations) : 1 }};
        ...
        let expCount = {{ count($doctor->experiences) > 0 ? count($doctor->experiences) : 1 }};
