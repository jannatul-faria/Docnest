            ...
            @else
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                    @foreach($wishlists as $wishlist)
                        ...
                        <div class="group bg-white rounded-[2.5rem] overflow-hidden border border-slate-100 hover:shadow-2xl transition-all duration-500 flex flex-col">
                            <!-- Image Area -->
                            <div class="relative aspect-[4/5] overflow-hidden bg-slate-50">
                                ...
                                @else
                                    <img src="{{ asset('assets/images/default-doctor.png') }}" class="w-full h-full object-cover group-hover:scale-110 transition-transform duration-700 opacity-80">
                                @endif
                                ...
                                <div onclick="removeFromWishlist(this, {{ $doctor->id }})" class="absolute top-4 right-4 h-10 w-10 bg-rose-500 text-white rounded-xl flex items-center justify-center shadow-lg cursor-pointer transition-all hover:bg-rose-600" title="Remove from Wishlist">
                                    <svg class="w-5 h-5" fill="currentColor" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z"></path></svg>
                                </div>
                            </div>
                            ...
                            <div class="p-6 flex-1 flex flex-col">
                                ...
                                <div class="mt-auto">
                                    <a href="{{ route('doctors.show', $doctor->id) }}" class="block w-full py-3.5 bg-primary text-white rounded-2xl font-black text-[10px] uppercase tracking-widest text-center hover:bg-primary/90 transition-all shadow-lg shadow-primary/10">
                                        View Profile
                                    </a>
                                </div>
                            </div>
